...
;by default the dot character class excludes newlines, so it won't
;match '\n' unless a certain switch known as 'single line mode' is 
;included.  In Perl this switch appears at the end of the regex as 
;an s, so the regex '/.{1}/s' would be just like this one, but 
;excludeNewlines would be false.  not singleLineMode means
;yes excludeNewLines
(define-fun dotCharClassAt((string0 String) (mIndex Int) (excludeNewLines Bool)) Bool
	(not
		(or
			(= (charOf string0 mIndex) _ERROR_)
			(and excludeNewLines (= (charOf string0 mIndex) _newline_))
		)
	)
)
...
(define-fun patternAt ((string0 String) (mIndex Int)) Bool
	(and 
		...
		(=> 
			...
			;then for all i within the bounds, we see the dot char class
...
(assert (= (charOf V1 3) _d_ ))
(assert (= (charOf V1 4) _e_ ))
(assert (= (charOf V1 5) _f_ ))
(assert (= (length V1) 6))
(check-sat)
(get-value (xIndex))
(pop)
...
;this V1 should not satisfy (all newlines)
(assert (= (charOf V1 0) _newline_ ))
(assert (= (charOf V1 1) _newline_ ))
(assert (= (charOf V1 2) _newline_ ))
(assert (= (charOf V1 3) _newline_ ))
(assert (= (length V1) 4))
(check-sat)
;(get-value (xIndex))

;this V1 should not satisfy (a non-char at the start)
(assert (= (charOf V1 0) _newline_ ))
(assert (= (charOf V1 1) _b_ ))
(assert (= (charOf V1 2) _b_ ))
(assert (= (charOf V1 3) _b_ ))
(assert (= (charOf V1 4) _c_ ))
(assert (= (charOf V1 5) _x_ ))
(assert (= (length V1) 6))
(check-sat)
;(get-value (xIndex))
