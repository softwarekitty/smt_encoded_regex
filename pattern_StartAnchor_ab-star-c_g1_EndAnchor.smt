...
(declare-const prefixLength Int)
(declare-const suffixLength Int)
(declare-const runLength Int)
...
;for all the indices starting at mIndex + prefixLength, but less than
...
(define-fun patternAt ((string0 String) (mIndex Int)) Bool
	...
	(and 
		...
		;we see an _a_ at the start of the match index (TODO - abstract to substringEqualAt function)
		(=  (charOf string0 mIndex) _a_ ) 
		
		;runLength can be from minRepetitions to maxRepetitions, which might vary for {N, }, +, *
		;default max is (length - (mIndex + prefixLength + suffixLength)) - the rest of available space
		(>= runLength minRepetitions)
		(<= runLength maxRepetitions)
		(<= runLength (- (length string0) (+ mIndex prefixLength suffixLength)))
		
		;if there is a run
		(=> 
			(> runLength 0) 
				
			;then for all i within the bounds, we see _b_
			(forall ((runIndexVar Int))
				(=>
					(and 
						(>= runIndexVar (+ mIndex prefixLength))
						(< runIndexVar (+ mIndex prefixLength runLength)) 
					)
					(= (charOf string0 runIndexVar) _b_)
				)
			)
		)
		
		;now having the run length is very handy for specifying the suffix
		(= (charOf string0 (+ mIndex prefixLength runLength)) _c_)
	)
)
...
(assert 
	(and
		(patternAt V1 xIndex) 
		(patternAt V1 (+ xIndex  prefixLength suffixLength runLength))
		(= xIndex 0)
	)
)
(push)
...
;this V1 should satisfy
(assert (= (charOf V1 0) _a_ ))
(assert (= (charOf V1 1) _b_ ))
(assert (= (charOf V1 2) _c_ ))
(assert (= (charOf V1 3) _a_ ))
(assert (= (charOf V1 4) _b_ ))
(assert (= (charOf V1 5) _c_ ))
(assert (= (length V1) 6))
(check-sat)
(get-value (xIndex))
(get-value (runLength)) ;1
(pop)
(push)

;this V1 should satisfy
(assert (= (charOf V1 0) _a_ ))
(assert (= (charOf V1 1) _c_ ))
(assert (= (charOf V1 2) _a_ ))
(assert (= (charOf V1 3) _c_ ))
(assert (= (length V1) 4))
(check-sat)
(get-value (xIndex))
(get-value (runLength)) ;0
(pop)
(push)

;this V1 not should satisfy (pattern starts late)
(assert (= (charOf V1 0) _x_ ))
(assert (= (charOf V1 1) _a_ ))
(assert (= (charOf V1 2) _b_ ))
(assert (= (charOf V1 3) _c_ ))
(assert (= (charOf V1 4) _a_ ))
(assert (= (charOf V1 5) _b_ ))
(assert (= (charOf V1 6) _c_ ))
(assert (= (charOf V1 7) _y_ ))
(assert (= (length V1) 8))
(check-sat)
(pop)
(push)

;this V1 should not satisfy
(assert (= (charOf V1 0) _r_ ))
(assert (= (charOf V1 1) _a_ ))
(assert (= (charOf V1 2) _c_ ))
(assert (= (charOf V1 3) _a_ ))
(assert (= (charOf V1 4) _c_ ))
(assert (= (charOf V1 5) _o_ ))
(assert (= (length V1) 6))
(check-sat)
(pop)
(push)

;this V1 should not satisfy - different numbers of b in first group
(assert (= (charOf V1 0) _a_ ))
(assert (= (charOf V1 1) _c_ ))
(assert (= (charOf V1 2) _a_ ))
...
;this V1 should not satisfy - no c in second group
(assert (= (charOf V1 0) _a_ ))
(assert (= (charOf V1 1) _c_ ))
(assert (= (charOf V1 2) _a_ ))
(assert (= (length V1) 3))
...
;this V1 should not satisfy - 2 then 1 b's
(assert (= (charOf V1 0) _a_ ))
(assert (= (charOf V1 1) _b_ ))
(assert (= (charOf V1 2) _b_ ))
(assert (= (charOf V1 3) _c_ ))
...
;this V1 should not satisfy - 0 then 4 b's
(assert (= (charOf V1 0) _a_ ))
(assert (= (charOf V1 1) _c_ ))
(assert (= (charOf V1 2) _a_ ))
(assert (= (charOf V1 3) _b_ ))
...
;this V1 should not satisfy - 1 then 2 b's
(assert (= (charOf V1 0) _a_ ))
(assert (= (charOf V1 1) _b_ ))
(assert (= (charOf V1 2) _c_ ))
(assert (= (charOf V1 3) _a_ ))
